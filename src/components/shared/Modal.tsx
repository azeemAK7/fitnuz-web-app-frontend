...
function Modal({ open, setOpen, children, title }: ModalProps) {
  return (
    <>
      <Dialog
        open={open}
        ...
        <div className="fixed inset-0 overflow-hidden">
          <div className="absolute inset-0 overflow-hidden">
            <div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
              <DialogPanel
                transition
                className="pointer-events-auto w-screen max-w-[800px] transform transition duration-500 ease-in-out data-closed:translate-x-full sm:duration-700"
              >
                  ...
                  <div className="relative mt-6 flex-1 p-8">
                    <div className="border-b pb-8 flex justify-between">
                      ...
                      <button onClick={() => setOpen(false)}>
                        <RxCross1 className="text-slate-600 hover:text-slate-800 text-2xl" />
                      </button>
                    </div>
                    {children}
                  </div>
